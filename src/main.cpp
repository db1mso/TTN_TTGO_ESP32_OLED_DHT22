...
// Erweitert um DHT22 und reduziert auf 1ch von Olli
...
void do_send(osjob_t* j){
    ...
    // static uint8_t message[] = "Hello from Olli!";

    // Check if there is not a current TX/RX job running
    if (LMIC.opmode & OP_TXRXPEND) {
        Serial.println(F("OP_TXRXPEND, not sending"));
    } else {
        // Code Example von TTN
        // read the values from the DHT22
        uint32_t humidity = dht.readHumidity(false) * 100; // mal 100 um die Dezimalstellen zu eliminieren
        uint32_t temperature = dht.readTemperature(false) * 100;

        // jetzt packen wir die Werte in die Bytes
        byte payload[4];
        ...
        display.drawString (0, 9, "Temperatur " + String (temperature) + "°C");
        display.drawString (0, 18, "Humidity " + String (humidity) + "%");
